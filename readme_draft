...
This is a Python script used in Sakata, Ishiguro, Mori et al (2020) to predict frequencies of base editing patterns for a given input sequence using a model trained using amplicon sequencing data obtained for a specific base editing method. Let <img src="https://latex.codecogs.com/svg.latex?\inline&space;s_i" title="s_i" /> be the nucleotide base transition status at <img src="https://latex.codecogs.com/svg.latex?\inline&space;i" title="i" /> bp position relative to the PAM at the target site and <img src="https://latex.codecogs.com/svg.latex?\inline&space;P(s_i)" title="P(s_i)" /> be the probability of <img src="https://latex.codecogs.com/svg.latex?\inline&space;s_i" title="s_i" />. A base editor model is prepared as a profile of <img src="https://latex.codecogs.com/svg.latex?\inline&space;P(s_i)" title="P(s_i)" /> and <img src="https://latex.codecogs.com/svg.latex?\inline&space;P(s_i|s_j)" title="P(s_i|s_j)" />  that can be prepared from amplicon sequence data of different target sites treated with the corresponding base editor (sample codes to generate a base editing model from amplicon data can be found in sample_training_codes/).
